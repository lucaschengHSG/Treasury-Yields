...
def get_tweets(usernames, number, since, until, language='en'):
    all_tweets = []  # List to hold all tweets from all usernames

    for username in usernames:
        try:
            # Fetch tweets for each username
            tweets = scraper.get_tweets(username, mode='user', number=number, since=since, until=until, language=language)

            # Check if tweets are present
            if 'tweets' in tweets and tweets['tweets']:
                for tweet in tweets['tweets']:
                    data = {'username': username, 'text': tweet['text'], 'created_at': tweet['date']}
                    all_tweets.append(data)
            else:
                print(f"No tweets found for {username} in the specified date range.")
        except Exception as e:
            print(f"An error occurred while fetching tweets for {username}: {e}")

    # Create a DataFrame from the collected tweets
    data = pd.DataFrame(all_tweets, columns=['username', 'text', 'created_at'])
    return data

since_date = '2023-09-01'
end_date = '2023-12-12'

# Define usernames of interest
usernames = ['GoldmanSachs', 'stlouisfed', 'federalreserve','Lagarde', 'RayDalio', 
             'BillAckman', 'MorganStanley','jpmorgan','BlackRock','BankofAmerica',
             'DeutscheBank', 'UBS', 'CNBC','Reuters', 'MorningstarInc', 'BusinessInsider',
             'WSJ', 'business', 'markets', 'economics', 'CNN', 'nytimes','BBCWorld',
             'Vanguard_Group', 'Fidelity', 'PIMCO','FinancialTimes','TheEconomist',
             'ecb', 'bankofengland','IMFNews', 'wef', 'WorldBank']

# Scrape the data for all the specified usernames
data = get_tweets(usernames, 1000, since_date, end_date, language='en')
...
data2 = data[data['text'].str.contains('|'.join(keywords), case=False, na=False)]

# Remove irrelevant and often present bitcoin data
data2 = data2[~data2['text'].str.contains('bitcoin|btc', case=False, na=False)]


# Check directory before extracting csv
data2.to_csv("Bond_Prices_smart.csv")

# View scraped data
tyields_data = pd.read_csv("Bond_Prices_smart.csv")
